...
void test9(){
    std::cout << "[Test 9]" << "\n";
    std::cout << "Testing disposePage() and testing readPage() when reading a disposed page." << "\n";
    bufMgr->disposePage(file7ptr, 1);
    try
    {
        bufMgr->readPage(file7ptr, 1,page);
        PRINT_ERROR("ERROR :: The page is invalid. Exception should have been thrown before execution reaches this point.");
    }
    catch (InvalidPageException & e) {}
    std::cout << "Passed" << "\n";
}
